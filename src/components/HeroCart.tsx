  ...
  return (
    <div>
      {cart.length === 0 ? (
        ...
        <div className=' px-2 lg:px-16 py-12 flex flex-col md:flex-row justify-between'>
          <div className='lg:max-w-[65%] pb-14 w-full relative '>
            <div className='h-[50px] bg-[#FFF9E5] w-full lg:flex lg:items-center lg:justify-center'>
              <ul className='flex justify-between w-full lg:gap-20 font-bold '>
                <li>Product</li>
                <li>Price</li>
                <li>Quantity</li>
                <li>Subtotal</li>
            ...
            {cart.map((item) => (
              <div key={item.id} className='flex justify-between items-center relative mt-[40px] '>
                <div className=" w-[75px] lg:w-[100px] bg-[#FBEBB5] rounded-lg">
                  <Image src={item.imagePath} alt="cart-items-image" width={200} height={200}
                    className="w-full h-full object-cover"
                  />
                </div>
                <h1 className="font-semibold text-[14px] lg:text-[16px] relative -left-7">{item.name}</h1>
                <p><span className="text-[#B88E2F] relative -left-[47px]">{item.price}</span></p>
                <div className='w-[50px] h-[50px] rounded-lg flex justify-center items-center relative -left-[50px]'>
                  {item.quantity}
                </div>
                <p className='relative -left-5'>{item.price * item.quantity}</p>
                <AiFillDelete className='w-10 h-10 text-[#FBEBB5] relative -left-8 cursor-pointer'
